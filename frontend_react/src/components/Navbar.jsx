  ...
  <nav className="sticky top-0 z-40 w-full md:w-64 md:h-screen bg-zinc-950/90 border-b md:border-b-0 md:border-r border-zinc-800/80 backdrop-blur-md shrink-0 flex flex-col">
   <div className="w-full px-3 sm:px-6 lg:px-5 h-16 md:h-full md:py-6 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-2 md:gap-8 overflow-y-auto no-scrollbar">
    ...
    {/* Global Multi-Search Input (Movies, Series, Anime, Actors) */}
     <div className="relative flex-1 md:w-full md:flex-none min-w-0 md:mt-2">
     ...
     {showDropdown && (
      <div className="absolute top-full md:top-auto md:bottom-auto left-0 md:left-full md:ml-4 right-0 md:w-96 mt-2 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in duration-100 max-h-[75vh] flex flex-col">
       {selectedPerson ? (
    ...
    {/* Right Actions */}
    <div className="flex flex-row md:flex-col items-center md:items-stretch justify-end md:justify-start gap-1.5 md:gap-2 shrink-0 md:w-full md:mt-auto border-t-0 md:border-t md:border-zinc-800/80 md:pt-6">
     {/* Desktop Library Links (Moved from top navbar) */}
     <div className="hidden md:flex flex-col gap-2 mb-4 w-full">
      ...
      <button
       ...
       className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-zinc-800/50 text-sm font-medium text-zinc-300 hover:text-white transition-all w-full"
      >
      ...
      <button
       onClick={onOpenDataModal}
       className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-zinc-800/50 text-sm font-medium text-zinc-400 hover:text-white transition-all w-full"
      >
     ...
     <form
      ...
      className="hidden lg:flex flex-col gap-2 mb-4 w-full px-2"
     >
     ...
     {user ? (
      <div className="relative flex items-center md:items-start gap-1 sm:gap-2 pl-1 sm:pl-0 md:w-full md:p-2 md:rounded-2xl md:bg-zinc-900/40 md:border md:border-zinc-800/80">
       <button
        onClick={() => setIsProfileMenuOpen(prev =>!prev)}
        className="flex items-center gap-1.5 sm:gap-3 p-0.5 md:p-1 w-full rounded-full md:rounded-xl hover:bg-zinc-800/50 transition-all text-left"
        title="Account Menu"
       >
        {user.photoURL ? (
         <img src={user.photoURL} alt={user.displayName} className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full border border-zinc-700 shrink-0 object-cover" />
        ) : (
         <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-rose-600/20 border border-rose-500/40 flex items-center justify-center text-xs md:text-sm font-medium text-rose-300 shrink-0">
       ...
       {isProfileMenuOpen && (
        <>
         ...
         <div className="absolute right-0 md:left-full md:right-auto md:-top-4 md:ml-4 top-full mt-2 w-64 p-3 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-150 flex flex-col gap-2.5">
      ...
      <div className="w-full flex items-center justify-end md:justify-center md:px-2 md:mt-2">
       <button
        onClick={onOpenAuthModal}
        className="flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-orange-600 hover:from-rose-500 hover:to-orange-500 text-white text-xs md:text-sm font-bold shadow-lg shadow-rose-950/40 transition-all active:scale-95 shrink-0 md:w-full"
       >
